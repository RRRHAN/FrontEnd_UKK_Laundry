...
const Paket = () => {
	...
	const [statePaket, setStatePaket] = useState(
		{ id: "", jenis: "", harga: 0 },
	)
	...
	const savePaket = (e) => {
		e.preventDefault()
		const id = statePaket.id ? statePaket.id : ""
		console.log(statePaket)
		let url = base_url + "/Paket/" + id
		const reqAPI = stateAction == "insert" ? axios.post : axios.put
		reqAPI(url, statePaket, headerConfig())
			.then((response) => {
				if (response.data.notLogged) {
					window.location = "/login"
				} else {
	...
	return (
		<>
			...
			<div class='grid grid-cols-3 gap-4'>
				<div>
					<h1 className='text-4xl m-10'>Paket</h1>
				</div>
				<div className='mx-auto mt-6'>
					<button
						onClick={() => {
							setModalIsOpen(true)
							setStatePaket({ id: "", jenis: "", harga: 0 })
								...
								<tbody class='bg-white divide-y divide-gray-200'>
									{statePakets.map((element, i) => (
										<tr>
											...
											<td class='px-6 py-4 whitespace-nowrap'>
												<div class='text-sm text-gray-900'>{element.jenis}</div>
											</td>
											<td class='px-6 py-4 whitespace-nowrap'>
												<div class='text-sm text-gray-900'>{element.harga}</div>
											</td>
			...
			>
				...
				<div className='block p-6 rounded-lg shadow-lg bg-slate-300 max-w-sm'>
					<form className='m-10'>
						<div className='form-group mb-6'>
							...
							<input
								value={statePaket.jenis}
								onChange={(event) => {
									let tempPaket = { ...statePaket }
									tempPaket.jenis = event.target.value
									setStatePaket(tempPaket)
								}}
								type='text'
						...
						<div className='form-group mb-6'>
							...
							<input
								value={statePaket.harga}
								onChange={(event) => {
									let tempPaket = { ...statePaket }
									tempPaket.harga = event.target.value
									setStatePaket(tempPaket)
								}}
								type='number'
								className='form-control block w-full px-3 py-1.5 text-base
